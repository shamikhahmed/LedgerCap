/** Shared morning-brief formatter — client (dynamic import) + worker cron. */
...
export function formatMorningBrief(brief, extras, fmtPkr) {
  extras = extras || {};
...
export function formatPortfolioDigest(row, fmtPkr) {
  const fmt = fmtPkr || ((n) => '₨' + Math.round(n || 0).toLocaleString('en-PK'));
...
export function formatMarketOpenDigest(extras) {
  const kse = extras?.kse100;
  const lines = ['🔔 *Market open — PSX*', ''];
  if (kse?.value) {
    const sign = (kse.changeP || 0) >= 0 ? '+' : '';
    lines.push(`KSE-100: *${Number(kse.value).toLocaleString('en-PK')}* (${sign}${Number(kse.changeP || 0).toFixed(2)}%)`);
  }
  (extras?.movers || []).slice(0, 6).forEach((m) => {
    lines.push(`• *${escapeMarkdown(m.symbol)}* ${m.movePct >= 0 ? '+' : ''}${Number(m.movePct).toFixed(1)}%`);
  });
  lines.push('', '_Rule-based — not financial advice._');
  return truncate(lines.join('\n'));
}

export function formatNewsDigest(news, title) {
...
export function formatPktTimestamp(iso) {
